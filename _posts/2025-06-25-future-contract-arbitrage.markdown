...
A forward contract is an agreement to buy(long) or sell(short) an underlying asset at future 
maturity date $t_{n}$ at a predetermined price F. The delivery of the asset takes place at time 
$t_{n}$ and consequently are required to buy/sell at the predetermined price F.

### Synthetic Forward Price

Synthetic forward price is the theoretical price of the forward contract that can be derived 
using the spot price of the **underlying asset** $S(t_{0})$ and **risk-free interest rate** $B(t_{0}, t_{n})$.

$$ F_{theoretical}(t_{0}) = B(t_{0}, t_{n}).S(t_{0}) $$

If the spot price of the forward contract is the same as the synthetic forward price there is 
no arbitrage([Eq-1](#fc_arbitrage)).

<div id="fc_arbitrage">Equation 1</div>

$$ F(t_{0}) = F_{theoretical}(t_{0}) $$

If we consider continuously compounded interest rate the risk-free interest rate is given by
$B(t_{0}, t_{n}) = e^{r \cdot (t_{n} - t_0)}$ and $B(t_{0}, t_{n}) = (1+\frac{r}{m})^{t_{n}}$ in 
...
* At $t_{0}$
    ...
    * invest $S(t_{0})$ in risk free asset till $t_{n}$
...
* At maturity $t_{n}$
    * Liquidate position from risk-free asset (risk-free asset more or less guarantees appreciation in asset)
        * Current Account = $+F_{theoretical}(t_{0}) = +B(t_{0}, t_{n}) \cdot S(t_{0})$
        * Liability = 1 share of underlying asset
...
* At maturity $t_{n}$
    ...
    * Borrowed $S(t_{0})$ has incurred interest r
        * Current Account = $F(t_{0})$
        * Liability = $-F_{theoretical}(t_{0}) = -B(t_{0}, t_{n}) \cdot S(t_{0})$
    * Pay back the cash received from bank with interest
        * Current Account = $\left[F(t_{0}) - F_{theoretical}(t_{0})\right] \rightarrow$ positive
        * Liability = 0
...
Buying a stock outright at time $t_{0}$ optionally come with dividends. Buying a forward contract at $t_{0}$ with maturity $t_{n}$ on the same 
stock doesn't come with any such obligation i.e. you miss out on the dividends between $t_{0}$ and $t_{n}$. Since you miss out on the dividends, 
the present value $D(t_{0})$ at $t_{0}$ of those dividends paid between $t_{0}$ and $t_{n}$ is discounted from the forward price. Dividends are usually not 
...
* Discrete payments: $F(t_{0}) = B(t_{0}, t_{n}) \cdot (S(t_{0}) - D(t_{0}))$
* Continuous payments: $F(t_{0}) = B(t_{0}, t_{n}) \cdot e^{-q \cdot (t_{n} - t_{0})} \cdot S(t_{0})$
...
exchange rate. Additionally, the value of the forward contract on a foreign currency is equated to 
...
$$ F_{theoretical}(t_{0}) = e^{r \cdot (t_{n} - t_{0})} \cdot S(t_{0}) $$

If we were to hold the foreign currency we would be get the benefits of the foreign risk-free 
...
risk-free interest rate $r_{f}$ between the period $t_{0}$ and $t_{n}$. This is discounted into 
the formula for the value for the forward contract as follows:

$$ F_{theoretical}(t_{0}) = e^{r \cdot (t_{n} - t_{0})} \cdot S(t_{0}) \cdot e^{-r_{f} \cdot (t_{n} - t_{0})}$$

$$ F_{theoretical}(t_{0}) = e^{(r - r_{f}) \cdot (t_{n} - t_{0})} \cdot S(t_{0}) $$

Similarly there would be arbitrage if $ F(t_{0}) = F_{theoretical}(t_{0}) $ doesn't hold true in the 
forward contracts on foregin currencies. Let's see an example
...
borrow 1 Euro at $t_{0}$ from the market we would owe $e^{r_{f} \cdot (t_{n} - t_{0})}$ at $t_{n}$, 
but we would only receive 1 Euro from the forward contract $\Rightarrow$ we wouldn't be able to close the short position. 
To equate this we would only borrow $e^{-r_{f} \cdot (t_{n} - t_{0})}$ at $t_{0}$ and invest into the local risk-free
asset. Here are the steps to make arbitrage profit from mispriced forward contract:

* At $t_{0}$
    * Buy forward contract on 1 Euro
        * Current Account = 0
        * Liability = 0
    * Borrow $e^{-r_{f} \cdot (t_{n} - t_{0})}$ Euro (or it's dollar equivalent $S(t_{0})$)
        * Current Account = $e^{-r_{f} \cdot (t_{n} - t_{0})} \cdot S(t_{0})$
        * Liability = -$e^{-r_{f} \cdot (t_{n} - t_{0})} \cdot S(t_{0})$
    * Invest the borrowed amount in local risk-free asset
        * Current Account = $e^{-r_{f} \cdot (t_{n} - t_{0})} \cdot S(t_{0})$ worth of U.S. treasury bond 
        * Liability = -$e^{-r_{f} \cdot (t_{n} - t_{0})} \cdot S(t_{0})$
* At maturity $t_{n}$
    * Liquidate U.S. treasury bond
        * Current Account = $F_{theoretical}(t_{0}) = e^{(r-r_{f}) \cdot (t_{n} - t_{0})} \cdot S(t_{0})$
        * Liability = -$S(t_{0})$
    ...
    * Close the short position and pay back $S(t_{0})$ amount to market
        * Current Account = $F_{theoretical}(t_{0}) - F(t_{0}) \rightarrow$ positive
        * Liability = 0
